...
export async function POST(req: Request) {
  try {
    ...
    if (useRag) {
      ...
      const collection = await astraDb.collection(`ecommerce_inventory`);
    ...
    const ragPrompt = [
      {
        role: 'system',
        content: `You are an AI assistant assisting customers to pick fashion products sold by ecommerce company. Include the product description, price when responding with the list of product recommendation. Answer question based on the context information from the inventory. Format responses using markdown where applicable. All the responses should be the same language as the user used.
        ${docContext} 
